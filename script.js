...
class Workout {
  ...
  _setDescription() {
    ...
    } on ${months[this.date.getMonth()]} ${this.date.getDate() + 1} `;
  }
  click() {
    this.clicks++;
  }
...
class App {
  ...
  _movToPopup(e) {
    ...
    if (!workoutEl) return;
    const targetedWorkout = this.#workouts.find(
      work => work.id === workoutEl.dataset.id
    );
    this.#map.setView(targetedWorkout.coords, this.#mapZoomLevel, {
      animate: true,
      pan: {
        duration: 1,
      },
    });
    targetedWorkout.click();
  }
}
